...
function create_release_notes() {
	...
	if [[ -z $commitMsgsSinceLastRelease ]]; then
	...
	IFS=$'\n'
	for commitMsg in $commitMsgsSinceLastRelease; do
		# Skip empty lines
		if [[ -z $commitMsg ]]; then
			continue
		fi
