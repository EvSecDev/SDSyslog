...
func watcher(ctx context.Context, logFileInput string, fileHasChanged chan bool, fileHasRotated chan bool) {
	...
	if err != nil {
		logctx.LogStdErr(ctx, "failed to add log file '%s' to inotify watcher: %w\n", logFileInput, err)
		return
	}
	...
	if err != nil {
		logctx.LogStdErr(ctx, "failed to add directory '%s' to inotify watcher: %w\n", logDirectory, err)
		return
	}
	...
	for {
		...
		default:
			...
			if err != nil {
				logctx.LogStdErr(ctx, "error reading inotify event: %w\n", err)
				continue
			}
			...
			for offset <= uint32(n)-syscall.SizeofInotifyEvent {
				...
				if err != nil {
					logctx.LogStdErr(ctx, "failed to read event content: %w\n", err)
					continue
				}
					...
					if (event.Mask & (syscall.IN_MOVED_FROM | syscall.IN_MOVED_TO | syscall.IN_DELETE | syscall.IN_CREATE)) != 0 {
						...
						for range maxRetries {
							...
							if !errors.Is(err, syscall.EACCES) && !errors.Is(err, syscall.EPERM) && !errors.Is(err, syscall.ENOENT) {
								logctx.LogStdErr(ctx, "failed to add rotated log file to inotify watcher: %w\n", err)
								break
							}
