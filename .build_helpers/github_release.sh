...
function create_github_release() {
	...
	if [[ -d $HOME/Downloads/releasetemp ]]; then
		localReleaseDir="$HOME/Downloads/releasetemp"
	elif [[ -d $HOME/.local/tmp/releasetemp ]]; then
		localReleaseDir="$HOME/.local/tmp/releasetemp"
	elif [[ -d /tmp/releasetemp ]]; then
	...
	if ! jq . <<<"$releaseMeta" >/dev/null; then
	...
	local localFileName
	while IFS= read -r -d '' localFileName; do
		...
		if [[ $uploadState != uploaded ]] || [[ $uploadState == null ]]; then
